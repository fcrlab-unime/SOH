...
def plot_generation_steps(similar_curves, interp_curve, final_curve, target_temp, target_soh):
    """
    Mostra un grafico di Nyquist che confronta le curve originali, la curva interpolata
    e la curva finale con rumore per un singolo campione generato.
    """
    fig, ax = plt.subplots(figsize=(12, 9))

    # 1. Plotta le curve originali simili (usate per l'interpolazione)
    for i, (_, real, imag) in enumerate(similar_curves):
        ax.plot(real, -imag, 'o--', color='gray', alpha=0.6, 
                label='Curve originali' if i == 0 else "")

    # 2. Plotta la curva interpolata (il risultato della media pesata)
    _, real_interp, imag_interp = interp_curve
    ax.plot(real_interp, -imag_interp, 's-', color='dodgerblue', markersize=6, 
            label='Curva Interpolata (Senza Rumore)')

    # 3. Plotta la curva sintetica finale (con l'aggiunta di rumore realistico)
    _, real_final, imag_final = final_curve
    ax.plot(real_final, -imag_final, 'o-', color='crimson', markersize=5, 
            label='Curva Finale Sintetica (con Rumore)')
    
    # Impostazioni del grafico
    ax.set_title(f"Processo di Generazione Curva Sintetica\nTarget: {target_temp}°C, {target_soh}% SOH", fontsize=16)
    ax.set_xlabel("Parte Reale (Z') [Ohm]", fontsize=12)
    ax.set_ylabel("- Parte Immaginaria (-Z'') [Ohm]", fontsize=12)
    ax.legend(fontsize=11)
    ax.grid(True, linestyle='--', alpha=0.7)
    ax.axis('equal')  # Fondamentale per la corretta visualizzazione dei Nyquist plots
...
def generate_synthetic_eis_dataset(df, n_cells=8, samples_per_cell=1200):
    ...
    for cell_id in range(n_cells):
        ...
        for i in range(samples_per_cell):
            ...
            plot_generation_steps(similar_curves=similar_curves, interp_curve=(freqs, real_interp, imag_interp), final_curve=(freqs, real_final, imag_final), target_temp=target_temp, target_soh=target_soh)
...
synthetic_df = generate_synthetic_eis_dataset(df, n_cells=8, samples_per_cell=1250)
    ...
    # Salva il dataset
synthetic_df.to_csv(f"synthetic_data_1.csv", index=False)
